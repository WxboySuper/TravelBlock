...
export function performSqliteInit(
  mod: ExpoSqliteModule | null | undefined,
  DB_NAME: string,
): SqliteDB | null {
  if (!mod) return null;
  const openSync = mod.openDatabaseSync;
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      db.transaction(
        (tx: SqliteTx) => {
          tx.executeSql(
            `CREATE TABLE IF NOT EXISTS ${TABLE_NAME} (key TEXT PRIMARY KEY NOT NULL, value TEXT)`,
          );
        },
